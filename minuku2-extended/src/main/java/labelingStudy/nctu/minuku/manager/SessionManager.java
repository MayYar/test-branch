...
//                SessionManager.instance = new SessionManager();
    ...
    public void addOngoingSessionid(int id) {
        labelingStudy.nctu.minuku.logger.Log.d(TAG, "test replay: adding ongonig session " + id );
    ...
    public void removeOngoingSessionid(String id) {
        labelingStudy.nctu.minuku.logger.Log.d(TAG, "test replay: inside removeongogint seesion renove " + id );
        this.mOngoingSessionIdList.remove(id);
        labelingStudy.nctu.minuku.logger.Log.d(TAG, "test replay: inside removeongogint seesion the ongoiong list is  " + mOngoingSessionIdList.toString() );
    }
...
//            data.add(new String[]{"timestamp","timeString","Latitude","Longitude","Accuracy"});
    ...
    public static boolean examineSessionCombinationByActivityAndTime(Session lastSession, String activity, long time){
        ...
        if (annotations.size() == 0) {
            labelingStudy.nctu.minuku.logger.Log.d(TAG, "[test combine] addSessionFlag = true  the last session is not the same activity");
            combine = false;
        }
        ...
        // the current activity is the same TM with the previous session mode, we check its time difference
            ...
            if (time - lastSession.getEndTime() <= SessionManager.SESSION_MIN_INTERVAL_THRESHOLD_TRANSPORTATION) {
                ...
                labelingStudy.nctu.minuku.logger.Log.d(TAG, "[test combine] the current activity is too close from the previous trip, continue the last session! the difference is "
        ...
        labelingStudy.nctu.minuku.logger.Log.d(TAG, "test combine: the previous acitivty is movnig,after combine it the last session is: " +  lastSessionStr );
        return combine;
    ...
    public static boolean isSessionLongEnough(String thresholdType, int sessionId){
        ...
        if (thresholdType.equals(SESSION_LONGENOUGH_THRESHOLD_DISTANCE)){
            ...
            //there are location records, we need to examine its distance
                ...
                labelingStudy.nctu.minuku.logger.Log.d(TAG, " test combine the distance between start and end is " + distance  + " now we comapre with the threshold " + SessionManager.SESSION_MIN_DISTANCE_THRESHOLD_TRANSPORTATION );
                ...
                if (distance<SessionManager.SESSION_MIN_DISTANCE_THRESHOLD_TRANSPORTATION){
                    labelingStudy.nctu.minuku.logger.Log.d(TAG, " test combine the trip is too short  ");
